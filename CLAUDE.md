...
**Every time Claude Code works on anything in this project, use the `superpowers` skill.**
This applies to all tasks — features, bug fixes, refactors, docs — no matter how small.
Invoke it before starting work.

## Session memory workflow (Claude_Logs)

This repo keeps a persistent Claude Code memory layer under **`Claude_Logs/`** so each fresh
session can recover context (it is documentation only — nothing there runs). See
`Claude_Logs/README.md` for the full workflow.

**Start every session by reading, in order:**

1. `CLAUDE.md` — this file: stable project rules and engineering priorities.
2. `Claude_Logs/CURRENT_CONTEXT.md` — the living summary of where things stand right now.
3. `Claude_Logs/SESSION_INDEX.md` — the index of all prior sessions.
4. The latest relevant log in `Claude_Logs/logs/`.

After reading those, state your understanding of the current state and your plan before editing.

**End every session by writing:**

1. A new log at `Claude_Logs/logs/YYYY-MM-DD_<short-task-slug>.md`, following
   `Claude_Logs/LOGGING_FORMAT.md` exactly (every heading, in order; write `None` where empty).
2. An **overwritten** `Claude_Logs/CURRENT_CONTEXT.md` (replace in place, do **not** prepend;
   keep ≤ ~150 lines).
3. A new row appended to `Claude_Logs/SESSION_INDEX.md`.

See `Claude_Logs/README.md` for the full workflow.
